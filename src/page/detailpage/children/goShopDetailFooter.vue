...
export default {
  ...
  methods: {
    ...
    async getAttention(){
      if(!$.cookie(keyConf.qm_cookie)){
        if (this.appOpen) { // APP中打开页面
          ...
          let baseUrl = stylist_id ? `/login?url=/qcsgoshopdetail/${this.productId}?stylist_id=${stylist_id}?plid=${this.plid}` : `/login?url=/qcsgoshopdetail/${this.productId}?plid=${this.plid}`;
    ...
    async cancle_attention (){
      if(!$.cookie(keyConf.qm_cookie)){
        this.$router.push({name: "login", query: {url:"/qcsgoshopdetail/" + this.productId}});
      }else{
