...
export default {
  ...
  methods: {
    ...
    async getAttention(){
      if(!$.cookie(keyConf.qm_cookie)){
        if (this.appOpen) { // APP中打开页面
          ...
          let baseUrl = stylist_id ? `/login?url=/shopservicedetail/${this.productId}?stylist_id=${stylist_id}?plid=${this.plid}` : `/login?url=/shopservicedetail/${this.productId}?plid=${this.plid}`;
    ...
    async cancle_attention (){
      if(!$.cookie(keyConf.qm_cookie)){
        this.$router.push({name: "login", query: {url:"/shopservicedetail/" + this.productId}});
      }else{
    ...
    async appiont(){
      ...
      if(!qm_cookie || isLogin.status == 'error'){
        ...
        let baseUrl;
        if (this.appOpen) { // APP中打开页面
          baseUrl = stylist_id ? `/shopserviceorder/${this.productId}?stylist_id=${stylist_id}&appOpen=true` : `/shopserviceorder/${this.productId}?appOpen=true`;
        }else {
          baseUrl = stylist_id ? `/shopserviceorder/${this.productId}?stylist_id=${stylist_id}` : `/shopserviceorder/${this.productId}`;
        }
        this.$router.push(baseUrl);
      }
    }
